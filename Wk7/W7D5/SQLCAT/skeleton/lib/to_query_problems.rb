...
def price_like_shiny_mouse
  ...
  WHERE toys1.price = toys2.price AND toys1.name != 'Shiny Mouse' AND toys2.name = 'Shiny Mouse'
  ORDER BY toys1.name;
  SQL
end
...
def just_like_orange
  ...
  # DO NOT USE A SUBQUERY
  execute(<<-SQL)
  SELECT cats1.name, cats1.breed 
  FROM cats AS cats1
  JOIN cats AS cats2 ON cats1.breed = cats2.breed
  WHERE cats1.breed = cats2.breed AND cats1.name != 'Orange' AND cats2.name = 'Orange'
  ORDER BY cats1.name;
  SQL
end
...
def toys_that_jet_owns
  ...
  # DO NOT USE A SUBQUERY
  execute(<<-SQL)
SELECT DISTINCT cat2.name, toys.name
FROM cats
JOIN cattoys ON cattoys.cat_id = cats.id
JOIN toys ON cattoys.toy_id = toys.id
JOIN cattoys AS cat1 ON cat1.toy_id = toys.id
JOIN cats AS cat2 ON cat1.cat_id = cat2.id
WHERE cats.name = 'Jet' AND cat2.name != 'Jet'
ORDER BY cat2.name;
